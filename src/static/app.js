    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.length
          ? details.participants.map(p => `<li><span class="participant-email">${p}</span><button class="remove-btn" title="Unregister" data-activity="${name}" data-email="${p}">&#x1F5D1;</button></li>`).join("")
